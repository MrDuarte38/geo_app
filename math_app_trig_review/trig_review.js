...
let answers = [22.2,29.1,58.1,65,18,17,84.3,232.6,26.1,6.3];

let problem_directions1 = "Find the missing side length. Round to the nearest tenth.";
let problem_directions2 = "Find the measure of the indicated angle to the nearest degree.";
let problem_directions3 = "Find the area of each figure. Round to the nearest tenth.";
let problem_directions4 = "Find the missing side length. Round to the nearest tenth.";
...
for(let i=0;i<problem_count;i++)
{
    if(i>=0 && i<=2)
    {
        let problem = new Problem(filenames[i],answers[i],problem_directions1);
        problem_list.push(problem);
    }
    else if(i>=3 && i<=5)
    {
        let problem = new Problem(filenames[i],answers[i],problem_directions2);
        problem_list.push(problem);
    }
    else if(i==6 || i==7)
    {
        let problem = new Problem(filenames[i],answers[i],problem_directions3);
        problem_list.push(problem);
    }
    else if(i>=8)
    {
        let problem = new Problem(filenames[i],answers[i],problem_directions4);
        problem_list.push(problem);
    }
...
direction_element.textContent = problem_directions1;
...
function next_clicked()
{
    ...
    direction_element.textContent = problem_list[current_problem].get_directions();
    erase();
    button_next.style.visibility = "hidden";
    input.value = "";
}
